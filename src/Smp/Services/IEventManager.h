...
namespace Services
{
static const Smp::String8 AlreadySubscribedTemplate
    = "Entry point '%s' of owner '%s' is already subscribed to event '%s'";
...
public:
    ...
    public:
        ...
        AlreadySubscribed(String8 _eventName, const IEntryPoint* _entryPoint) throw()
            : Smp::Exception("AlreadySubscribed"), eventName(_eventName), entryPoint(_entryPoint)
        {
            ...
            description
                = new Char8[std::strlen(AlreadySubscribedTemplate) + std::strlen(entryPointName) + std::strlen(ownerName) + std::strlen(eventName)];

            std::sprintf(description, AlreadySubscribedTemplate, entryPointName, ownerName, eventName); // NOLINT
        }
    ...
    public:
        ...
        NotSubscribed(String8 _eventName, const IEntryPoint* _entryPoint) throw()
            : Smp::Exception("NotSubscribed"), eventName(_eventName), entryPoint(_entryPoint)
        {
            Smp::String8 entryPointName = "";
            Smp::String8 ownerName = "";
            ...
            description
                = new Char8[std::strlen(NotSubscribedTemplate) + std::strlen(entryPointName) + std::strlen(ownerName) + std::strlen(eventName)];

            std::sprintf(description, NotSubscribedTemplate, entryPointName, ownerName, eventName); // NOLINT
        }
    ...
    virtual void Emit(EventId event) = 0;
};
}
}
