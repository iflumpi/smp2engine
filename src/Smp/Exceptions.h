...
protected:
    ...
    Exception(String8 _exceptionName) throw() : exceptionName(_exceptionName), description(nullptr)
    {
    }
...
public:
    // Added by htp (Spacebel): to avoid memory corruption
    /// Copy constructor.
    Exception(const Exception& ex) throw() : exceptionName(nullptr), description(nullptr)
    {
        exceptionName = ex.GetName();
        description = new Char8[std::strlen(ex.GetDescription()) + 1];
        std::strcpy(description, ex.GetDescription());
    }
    ...
    // Implement std::exception interface
...
public:
    ...
    InvalidObjectName(String8 _objectName) : Exception("InvalidObjectName"), objectName(_objectName)
    {
        description = new Char8[std::strlen(InvalidObjectNameTemplate) + std::strlen(objectName)];

        std::sprintf(description, InvalidObjectNameTemplate, objectName); // NOLINT
    }
    ~InvalidObjectName() throw() = default;
};
}
...
public:
    ...
    /// Constructor for new exception.
    DuplicateName(String8 _name) throw() : Exception("DuplicateName"), name(_name)
    {
        description = new Char8[std::strlen(DuplicateNameTemplate) + std::strlen(name)];

        std::sprintf(description, DuplicateNameTemplate, name); // NOLINT
    }
    ~DuplicateName() throw() = default;
};
}
...
public:
    ...
    /// Constructor for new exception.
    InvalidAnyType(Smp::SimpleTypeKind _invalidType, Smp::SimpleTypeKind _expectedType) throw()
        : Exception("InvalidAnyType"), invalidType(_invalidType), expectedType(_expectedType)
    {
        description
            = new Char8[std::strlen(InvalidAnyTypeTemplate) + std::strlen(TypeName[expectedType]) + std::strlen(TypeName[invalidType])];

        std::sprintf(description, InvalidAnyTypeTemplate, TypeName[expectedType], TypeName[invalidType]); // NOLINT
    }
    ~InvalidAnyType() throw() = default;
};
}
...
public:
    ...
    InvalidObjectType(IObject* _invalidObject) : Exception("InvalidObjectType"), invalidObject(_invalidObject)
    {
        ...
        description = new Char8[std::strlen(InvalidObjectTypeTemplate) + std::strlen(objectName)];

        std::sprintf(description, InvalidObjectTypeTemplate, objectName); // NOLINT
    }
    ~InvalidObjectType() throw() = default;
};
}
...
// SPR-294 String8 set as last item in array TypeName, to avoid mismatches with
// the SimpleTypeKind enum.
//
