...
public:
    ...
    public:
        ...
        InvalidModelState(Smp::ModelStateKind _invalidState, Smp::ModelStateKind _expectedState) throw()
            : Smp::Exception("InvalidModelState"), invalidState(_invalidState), expectedState(_expectedState)
        {
            description = new Char8[std::strlen(InvalidModelStateTemplate) + std::strlen(ModelStateName[expectedState])
                                    + std::strlen(ModelStateName[invalidState])];

            std::sprintf(description, InvalidModelStateTemplate, ModelStateName[expectedState], ModelStateName[invalidState]); // NOLINT
        }
        ~InvalidModelState() throw() = default;
    };
