...
public:
    ...
    public:
        ...
        DuplicateUuid(String8 _newName, String8 _oldName) throw()
            : Smp::Exception("DuplicateUuid"), newName(_newName), oldName(_oldName)
        {
            description = new Char8[std::strlen(DuplicateUuidTemplate) + std::strlen(newName) + std::strlen(oldName)];

            std::sprintf(description, DuplicateUuidTemplate, newName, oldName); // NOLINT
        }
        ~DuplicateUuid() throw() = default;
    };
    ...
    virtual FactoryCollection GetFactories(Uuid specUuid) = 0;
};
}
