...
namespace Publication
{
static const Smp::String8 NotRegisteredTemplate
    = "Type with Uuid = {%08X-%04X-%04X-%02X%02X-%02X%02X%02X%02X%02X%02X} has "
      "not been registered";
...
public:
    ...
    NotRegistered(Uuid _uuid) throw() : Smp::Exception("NotRegistered"), uuid(_uuid)
    {
        description = new Char8[std::strlen(NotRegisteredTemplate)];

        std::sprintf(description, NotRegisteredTemplate, // NOLINT
                     uuid.Data1, uuid.Data2, uuid.Data3, uuid.Data4[0], uuid.Data4[1], uuid.Data4[2], uuid.Data4[3],
                     uuid.Data4[4], uuid.Data4[5], uuid.Data4[6], uuid.Data4[7]);
    }
    ~NotRegistered() throw() = default;
};
}
}
...
public:
    ...
    AlreadyRegistered(String8 _name, IType* _type) throw()
        : Smp::Exception("AlreadyRegistered"), name(_name), type(_type)
    {
        description = new Char8[std::strlen(AlreadyRegisteredTemplate) + std::strlen(name) + std::strlen(type->GetName())];

        std::sprintf(description, AlreadyRegisteredTemplate, name, type->GetName()); // NOLINT
    }
    ~AlreadyRegistered() throw() = default;
};
}
}
...
public:
    ...
    virtual IType* GetType(SimpleTypeKind type) const = 0;
    ...
    virtual const IType* AddFloatType(String8 name,
                                      String8 description,
                                      Uuid typeUuid,
                                      Float64 minimum,
                                      Float64 maximum,
                                      ...
                                      Bool maxInclusive,
                                      String8 unit,
                                      SimpleTypeKind type = ST_Float64)
        = 0;
    ...
    virtual const IType* AddIntegerType(String8 name, String8 description, Uuid typeUuid, Int64 minimum, Int64 maximum, SimpleTypeKind type = ST_Int32)
        = 0;
    ...
    virtual IEnumerationType* AddEnumerationType(String8 name, String8 description, Uuid typeUuid, Int16 memorySize) = 0;
    ...
    /// @remarks None.
    virtual const IType* AddArrayType(String8 name, String8 description, Uuid typeUuid, Uuid itemTypeUuid, Int64 itemSize, Int64 arrayCount)
        = 0;
    ...
    /// @return  Interface to new type.
    /// @remarks None.
    virtual const IType* AddStringType(String8 name, String8 description, Uuid typeUuid, Int64 length) = 0;
    ...
    /// @remarks None.
    virtual IStructureType* AddStructureType(String8 name, String8 description, Uuid typeUuid) = 0;
    ...
    ///          operations.
    /// @remarks None.
    virtual IClassType* AddClassType(String8 name, String8 description, Uuid typeUuid, Smp::Uuid baseClassUuid) = 0;
};
}
}
