...
static const Smp::String8 InvalidParameterCountTemplate
    = "Invalid parameter count of operation '%s': Expected '%d', but found '%d'";
static const Smp::String8 InvalidParameterTypeTemplate
    = "Invalid parameter '%s' of operation '%s': Expected '%s', but found '%s'";
...
public:
    ...
    public:
        ...
        InvalidOperationName(String8 _operationName) throw()
            : Smp::Exception("InvalidOperationName"), operationName(_operationName)
        {
            description = new Char8[std::strlen(InvalidOperationNameTemplate) + std::strlen(operationName)];

            std::sprintf(description, InvalidOperationNameTemplate, operationName); // NOLINT
        }
        ~InvalidOperationName() throw() = default;
    };
    ...
    public:
        ...
        InvalidParameterCount(String8 _operationName, const Int32 _operationParameters, const Int32 _requestParameters) throw()
            : Smp::Exception("InvalidParameterCount")
            , operationName(_operationName)
            , operationParameters(_operationParameters)
            , requestParameters(_requestParameters)
        {
            description = new Char8[std::strlen(InvalidParameterCountTemplate) + std::strlen(operationName) + 8 + 8];

            std::sprintf(description, InvalidParameterCountTemplate, operationName, operationParameters, requestParameters); // NOLINT
        }
        ~InvalidParameterCount() throw() = default;
    };
    ...
    public:
        ...
        InvalidParameterType(String8 _operationName,
        ...
        {
            description = new Char8[std::strlen(InvalidParameterTypeTemplate) + std::strlen(parameterName) + std::strlen(operationName)
                                    + std::strlen(TypeName[expectedType]) + std::strlen(TypeName[invalidType])];

            std::sprintf(description, // NOLINT
                         InvalidParameterTypeTemplate, parameterName, operationName, TypeName[expectedType],
                         TypeName[invalidType]);
        }
        ~InvalidParameterType() throw() = default;
    };
