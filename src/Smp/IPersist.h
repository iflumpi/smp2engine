...
public:
    ...
    /// NOTE: OHB added 'const' for address. This was not in the SMP2 technical notes.
    virtual void Store(const void* address, Int32 size) = 0;
};
}
...
public:
    ...
    public:
        ...
        CannotRestore(String8 _message) throw() : Smp::Exception("CannotRestore"), message(_message)
        {
            description = new Char8[std::strlen(CannotRestoreTemplate) + std::strlen(message)];

            std::sprintf(description, CannotRestoreTemplate, message); // NOLINT
        }
        ~CannotRestore() throw() = default;
    };
    ...
    public:
        ...
        CannotStore(String8 _message) throw() : Smp::Exception("CannotStore"), message(_message)
        {
            description = new Char8[std::strlen(CannotStoreTemplate) + std::strlen(message)];

            std::sprintf(description, CannotStoreTemplate, message); // NOLINT
        }
        ~CannotStore() throw() = default;
    };
