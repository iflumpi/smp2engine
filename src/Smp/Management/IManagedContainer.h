...
public:
    ...
    public:
        ...
        ContainerFull(String8 _containerName, const Int64 _containerSize) throw()
            : Smp::Exception("ContainerFull"), containerName(_containerName), containerSize(_containerSize)
        {
            std::stringstream ss;

            ss << "Container '" << containerName << "' is full (" << containerSize << " components)";

            description = new Char8[ss.str().length() + 1];

            std::strcpy(description, ss.str().c_str());
        }
        ~ContainerFull() throw() = default;
    };
