...
class IManagedModel : public virtual IManagedComponent, public virtual IModel
{
public:
    /// Invalid field name.
    ...
    public:
        ...
        InvalidFieldName(String8 _fieldName) throw() : Smp::Exception("InvalidFieldName"), fieldName(_fieldName)
        {
            description = new Char8[std::strlen(InvalidFieldNameTemplate) + std::strlen(fieldName)];

            std::sprintf(description, InvalidFieldNameTemplate, fieldName); // NOLINT
        }
        ~InvalidFieldName() throw() = default;
    };
    ...
    public:
        ...
        InvalidFieldValue(String8 _fieldName, const AnySimple& _invalidValue) throw()
            : Smp::Exception("InvalidFieldValue"), fieldName(_fieldName), invalidValue(_invalidValue)
        {
            description = new Char8[std::strlen(InvalidFieldValueTemplate) + std::strlen(fieldName)];

            std::sprintf(description, InvalidFieldValueTemplate, fieldName); // NOLINT
        }

        virtual ~InvalidFieldValue() throw() = default;
    };
    ...
    public:
        ...
        InvalidArraySize(String8 _fieldName, const Int64 _givenSize, const Int64 _arraySize) throw()
            : Smp::Exception("InvalidArraySize"), fieldName(_fieldName), givenSize(_givenSize), arraySize(_arraySize)
        {
            std::stringstream ss;
            ss << "Invalid size for array '" << fieldName << "': Expected " << arraySize << ", but found " << givenSize;

            description = new Char8[ss.str().length() + 1];

            std::strcpy(description, ss.str().c_str());
        }
        ~InvalidArraySize() throw() = default;
    };
    ...
    public:
        ...
        String8 fieldName;
        /// Value that was passed as new field value.
        const AnySimpleArray invalidValue; // NOLINT(misc-misplaced-const)

        InvalidArrayValue(String8 _fieldName,
                          const AnySimpleArray _invalidValue) // NOLINT(misc-misplaced-const)
            throw()
            : Smp::Exception("InvalidArrayValue"), fieldName(_fieldName), invalidValue(_invalidValue)
        {
            description = new Char8[std::strlen(InvalidArrayValueTemplate) + std::strlen(fieldName)];

            std::sprintf(description, InvalidArrayValueTemplate, fieldName); // NOLINT
        }
        ~InvalidArrayValue() throw() = default;
    };
...
// Some message formatting reworked to be portable and without compile-time
// warnings.
//
