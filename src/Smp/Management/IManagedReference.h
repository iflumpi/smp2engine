...
public:
    ...
    public:
        ...
        ReferenceFull(String8 _referenceName, const Int64 _referenceSize) throw()
            : Smp::Exception("ReferenceFull"), referenceName(_referenceName), referenceSize(_referenceSize)
        {
            std::stringstream ss;

            ss << "Reference '" << referenceName << "' is full (" << referenceSize << "components)";

            description = new Char8[ss.str().length() + 1];

            std::strcpy(description, ss.str().c_str());
        }
        ~ReferenceFull() throw() = default;
    };
    ...
    public:
        ...
        NotReferenced(String8 _referenceName, const IComponent* _component) throw()
            : Smp::Exception("NotReferenced"), referenceName(_referenceName), component(_component)
        {
            ...
            description = new Char8[std::strlen(NotReferencedTemplate) + std::strlen(referenceName) + std::strlen(componentName)];

            std::sprintf(description, NotReferencedTemplate, referenceName, componentName); // NOLINT
        }
        ~NotReferenced() throw() = default;
    };
